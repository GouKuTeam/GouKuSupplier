    ...
    if (![textField.text isEqualToString:@""]) {
        [CommodityHandler getCommodityInformationWithBarCode:textField.text prepare:nil success:^(id obj) {
            ...
            if ([[dic objectForKey:@"errCode"] intValue] == 0) {
                SupplierCommodityEndity *entity = [SupplierCommodityEndity parseSupplierCommodityEndityWithJson:[dic objectForKey:@"data"]];
                //添加商品
                ...
                vc.addCustomCommodityComplete = ^(SupplierCommodityEndity *entity) {
                    self.tfsousuo.text = @"";
                    [self.arr_commodity removeAllObjects];
                    [self.arr_commodity addObject:entity];
                    [self.tb_commodity reloadData];
