...
- (void)onCreate{
    ...
    self.v_commodityView.v_commodityName.tf_detail.keyboardType = UIKeyboardTypeDefault;
    self.v_commodityView.v_commodityDescribe.tf_detail.enabled = YES;
    self.v_commodityView.v_commodityDescribe.tf_detail.keyboardType = UIKeyboardTypeDefault;
...
- (void)tgr_editPriceAction{
    ...
    vc.goBackAddSupplierCommodity = ^(NSDictionary *dic) {
        ...
        if (self.scEntity.defaultUsing == 1) {
...
- (void)rightBarAction{
    ...
    if (self.imgData == nil) {
        ...
        NSLog(@"不空");
        pictures = [NSString stringWithFormat:@"%@-%@",[LoginStorage GetShopId],self.barcode];
    }
    
    
    [CommodityHandler addCustomizeCommodityWithName:self.v_commodityView.v_commodityName.tf_detail.text barcode:self.barcode description:self.v_commodityView.v_commodityDescribe.tf_detail.text firstCategoryId:self.shopCId xprice:self.v_commodityView.v_jinhuoPrice.tf_detail.text stock:[NSNumber numberWithInt:[self.v_commodityView.v_stock.tf_detail.text intValue]] pictures:pictures price:[self.dic_price objectForKey:@"price"] isusing:[[self.dic_price objectForKey:@"using"] boolValue] saleUnits:[self.dic_price objectForKey:@"saleUnits"] prepare:^{
        
    } success:^(id obj) {
        SupplierCommodityEndity *entity = [SupplierCommodityEndity parseSupplierCommodityEndityWithJson:(NSDictionary *)obj];
